package com.hrs.paymentservice.configurations;
...
import com.hrs.paymentservice.models.ApiErrorResponse;
...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<ApiErrorResponse> handlePaymentNotFoundExceptions(final PaymentNotFoundException ex,
			final HttpServletRequest request) {

		List<String> errors = Collections.singletonList(ex.getMessage());

		ApiErrorResponse response = new ApiErrorResponse(HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND.name(),
				request.getRequestURI(), request.getMethod(), LocalDateTime.now(), "payment-not-found", errors);
